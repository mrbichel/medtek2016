...
HashMap<String,Integer> imgIndex = new HashMap<String,Integer>();
PImage[] img;
int img_next = 0;
int uge = 3;
String parti = "DF";
...
void reload(int uge, String parti){
  ...
  if ( db.connect() )
  {
    String Q = "select text, profile_image_url from st natural join users where parti='"+parti+"' and weekno="+uge;
    db.query(Q);
    println(Q);
    while (db.next ())
    {
      Tweet tweet = new Tweet(db.getString("text"),db.getString("profile_image_url"));
      tweets.add(tweet);
    }
    println(tweets.size());
  }
...
int insertImage(String url) {
  if (imgIndex.containsKey(url)) {
    return imgIndex.get(url);
  } else {
    PImage newImg = loadImage(url);
    imgIndex.put(url, img_next);
    println("index ",img_next);
    img[img_next] = newImg;
    img_next++;
    return img_next;
  }
